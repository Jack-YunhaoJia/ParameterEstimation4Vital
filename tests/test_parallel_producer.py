...
class TestCheckpointRoundTrip:
    ...
    def test_save_and_load_basic(self, tmp_path: Path) -> None:
        ...
        loaded, retry_state = producer._load_checkpoint(checkpoint_path)
...
class TestLoadCheckpointErrors:
    ...
    def test_missing_file(self, tmp_path: Path) -> None:
        """文件不存在时返回空列表。"""
        producer = _make_producer()
        loaded, retry_state = producer._load_checkpoint(tmp_path / "nonexistent.json")
        assert loaded == []
        assert retry_state is None
...
class TestRenderParallel:
    ...
    def test_status_updates(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        def mock_render_worker(tasks, vital_vst_path, adaptive_config_dict=None):
...
class TestSaveProductionHDF5:
    ...
    def _make_test_data(n: int = 100):
        ...
        sample_ids = [f"preset_{i:05d}_C3_v80" for i in range(n)]
        return params, embeddings, midi_conditions, audio_stats, metadata, config_yaml, sample_ids
    ...
    def test_creates_correct_hdf5_structure(self, tmp_path: Path) -> None:
        ...
        producer = _make_producer()
        params, embeddings, midi_conds, audio_stats, meta, cfg, sample_ids = self._make_test_data(100)
        hdf5_path = tmp_path / "test.h5"

        producer.save_production_hdf5(
            hdf5_path, params, sample_ids, embeddings, midi_conds, audio_stats, meta, cfg
        )
    ...
    def test_data_shapes_and_types(self, tmp_path: Path) -> None:
        ...
        producer = _make_producer()
        n = 100
        params, embeddings, midi_conds, audio_stats, meta, cfg, sample_ids = self._make_test_data(n)
        hdf5_path = tmp_path / "test.h5"
    ...
    def test_metadata_fields(self, tmp_path: Path) -> None:
        ...
        producer = _make_producer()
        params, embeddings, midi_conds, audio_stats, meta, cfg, sample_ids = self._make_test_data(50)
        hdf5_path = tmp_path / "test.h5"
...
# ============================================================
# Bug Condition 探索性测试：save_production_hdf5 缺失 sample_ids
# ============================================================

class TestBugConditionSampleIdsMissing:
    """Bug Condition 探索性测试。

    验证 save_production_hdf5() 是否在 HDF5 的 train/val/test 分组中
    写入 sample_ids 数据集。在未修复代码上此测试预期 FAIL。

    **Validates: Requirements 1.1, 1.2, 1.3, 2.1, 2.3**
    """

    def test_bug_condition_sample_ids_missing(self, tmp_path: Path) -> None:
        """save_production_hdf5 应接受 sample_ids 参数并写入 HDF5。

        Bug Condition: save_production_hdf5() 不接受 sample_ids 参数，
        HDF5 中 train/val/test 分组不包含 sample_ids 数据集。

        此测试在未修复代码上预期 FAIL（TypeError 或 AssertionError），
        修复后预期 PASS。
        """
        import h5py
        ...
        producer = _make_producer()
        n = 100
        params, embeddings, midi_conds, audio_stats, meta, cfg, sample_ids = (
            TestSaveProductionHDF5._make_test_data(n)
        )
        sample_ids = [f"preset_{i:05d}_C3_v80" for i in range(n)]

        hdf5_path = tmp_path / "test_bug_condition.h5"

        # 调用 save_production_hdf5 并传入 sample_ids 参数
        # 在未修复代码上，此调用会因 TypeError（不接受 sample_ids）而失败
        producer.save_production_hdf5(
            output_path=hdf5_path,
            params=params,
            sample_ids=sample_ids,
            embeddings=embeddings,
            midi_conditions=midi_conds,
            audio_stats=audio_stats,
            metadata=meta,
            config_yaml=cfg,
        )

        # 验证 HDF5 中每个 split 包含 sample_ids 数据集
        with h5py.File(str(hdf5_path), "r") as f:
            all_ids_in_splits: list[str] = []

            for split in ["train", "val", "test"]:
                grp = f[split]

                # 断言：sample_ids 数据集存在
                assert "sample_ids" in grp, (
                    f"HDF5 分组 '{split}' 中缺少 'sample_ids' 数据集"
                )

                # 断言：sample_ids 长度与 params 行数一致
                n_split = grp["params"].shape[0]
                assert len(grp["sample_ids"]) == n_split, (
                    f"'{split}' 中 sample_ids 长度 ({len(grp['sample_ids'])}) "
                    f"与 params 行数 ({n_split}) 不一致"
                )

                # 收集所有 sample_ids
                split_ids = [
                    x.decode() if isinstance(x, bytes) else x
                    for x in grp["sample_ids"][:]
                ]
                all_ids_in_splits.extend(split_ids)

            # 断言：所有 sample_ids 在三个分组中恰好出现一次（无丢失、无重复）
            assert sorted(all_ids_in_splits) == sorted(sample_ids), (
                "sample_ids 在 train/val/test 分组中存在丢失或重复"
            )


# ============================================================
# Preservation 属性测试：现有数据集和划分逻辑不变
# ============================================================
...
class TestPreservationProperty:
    ...
    验证 save_production_hdf5() 对现有数据集（params、embeddings、midi、
    audio_stats）的写入行为和 80/10/10 划分逻辑在任意样本数量下保持不变。
    ...
    **Validates: Requirements 3.1, 3.2, 3.3**
    ...
    def test_preservation_property(self, n: int, tmp_path_factory) -> None:
        """对随机 n，验证数据形状、类型、划分比例和 metadata 结构不变。
        ...
        **Validates: Requirements 3.1, 3.2, 3.3**
        ...
        expected_val = int(n * 0.1)
        expected_test = n - expected_train - expected_val

        with h5py.File(str(hdf5_path), "r") as f:
            # --- 3.2: 验证 80/10/10 划分样本数量 ---
            n_train = f["train/params"].shape[0]
            n_val = f["val/params"].shape[0]
            n_test = f["test/params"].shape[0]

            assert n_train == expected_train, (
                f"train 样本数 {n_train} != 期望 {expected_train} (n={n})"
            )
            assert n_val == expected_val, (
                f"val 样本数 {n_val} != 期望 {expected_val} (n={n})"
            )
            assert n_test == expected_test, (
                f"test 样本数 {n_test} != 期望 {expected_test} (n={n})"
            )

            # 所有 split 的 params 行数之和等于 n
            assert n_train + n_val + n_test == n

            # --- 3.1: 验证各 split 数据形状和类型不变 ---
            for split in ["train", "val", "test"]:
                grp = f[split]
                n_split = grp["params"].shape[0]

                # params: (n_split, 45), float32
                assert grp["params"].shape == (n_split, 45)
                assert grp["params"].dtype == np.float32

                # embeddings: (n_split, 1024), float32
                assert grp["embeddings"].shape == (n_split, 1024)
                assert grp["embeddings"].dtype == np.float32

                # midi_notes: (n_split,), int32
                assert grp["midi_notes"].shape == (n_split,)
                assert grp["midi_notes"].dtype == np.int32

                # midi_velocities: (n_split,), int32
                assert grp["midi_velocities"].shape == (n_split,)
                assert grp["midi_velocities"].dtype == np.int32

                # midi_durations: (n_split,), float32
                assert grp["midi_durations"].shape == (n_split,)
                assert grp["midi_durations"].dtype == np.float32
                ...
                assert "audio_stats" in grp
                assert grp["audio_stats/original_rms"].shape == (n_split,)
                assert grp["audio_stats/original_rms"].dtype == np.float32
                assert grp["audio_stats/original_peak"].shape == (n_split,)
                assert grp["audio_stats/original_peak"].dtype == np.float32
                assert grp["audio_stats/clipping_ratio"].shape == (n_split,)
                assert grp["audio_stats/clipping_ratio"].dtype == np.float32
            ...
            # param_names: 45 个字符串
            assert "param_names" in meta_grp
            param_names = meta_grp["param_names"][:]
            assert len(param_names) == 45
            ...
            # 属性字段
            assert "sampling_strategy" in meta_grp.attrs
            assert "seed" in meta_grp.attrs
            assert "production_timestamp" in meta_grp.attrs
            assert "vital_version" in meta_grp.attrs
            assert "production_config" in meta_grp.attrs
...
# ============================================================
# 测试 ProductionConfig 自适应渲染新字段
# ============================================================

class TestProductionConfigAdaptive:
    """ProductionConfig 自适应渲染字段默认值和 YAML 配置加载测试。"""

    def test_default_adaptive_rendering_disabled(self) -> None:
        """默认自适应渲染应为关闭状态。"""
        config = ProductionConfig()
        assert config.adaptive_rendering is False

    def test_default_sustain_margin(self) -> None:
        """默认 sustain_margin 应为 0.2。"""
        config = ProductionConfig()
        assert config.sustain_margin == 0.2

    def test_default_tail_margin(self) -> None:
        """默认 tail_margin 应为 0.1。"""
        config = ProductionConfig()
        assert config.tail_margin == 0.1

    def test_default_target_length_sec_none(self) -> None:
        """默认 target_length_sec 应为 None。"""
        config = ProductionConfig()
        assert config.target_length_sec is None

    def test_default_max_duration_sec(self) -> None:
        """默认 max_duration_sec 应为 30.0。"""
        config = ProductionConfig()
        assert config.max_duration_sec == 30.0

    def test_custom_adaptive_fields(self) -> None:
        """自定义自适应渲染字段。"""
        config = ProductionConfig(
        ...
        assert config.adaptive_rendering is True
        assert config.sustain_margin == 0.3
        assert config.tail_margin == 0.2
        assert config.target_length_sec == 5.0
        assert config.max_duration_sec == 20.0

    def test_yaml_config_maps_to_production_config(self) -> None:
        """YAML 配置文件中的 adaptive_rendering 段应正确映射到 ProductionConfig。"""
        yaml_content = """
adaptive_rendering:
  enabled: true
  sustain_margin: 0.3
...
"""
        ...
        ar = data["adaptive_rendering"]

        config = ProductionConfig(
            adaptive_rendering=ar["enabled"],
            sustain_margin=ar["sustain_margin"],
            tail_margin=ar["tail_margin"],
            target_length_sec=ar["target_length_sec"],
            max_duration_sec=ar["max_duration_sec"],
        )
        assert config.adaptive_rendering is True
        assert config.sustain_margin == 0.3
        assert config.tail_margin == 0.15
        assert config.target_length_sec == 4.0
        assert config.max_duration_sec == 25.0

    def test_yaml_config_null_target_length(self) -> None:
        """YAML 中 target_length_sec: null 应映射为 None。"""
        yaml_content = """
adaptive_rendering:
  enabled: false
  sustain_margin: 0.2
...
"""
    ...
    def test_production_yaml_loads_adaptive_section(self) -> None:
        """configs/production.yaml 应包含 adaptive_rendering 配置段。"""
        import yaml
        with open("configs/production.yaml", "r", encoding="utf-8") as f:
            data = yaml.safe_load(f)

        assert "adaptive_rendering" in data
        ar = data["adaptive_rendering"]
        assert ar["enabled"] is False
        assert ar["sustain_margin"] == 0.2
        assert ar["tail_margin"] == 0.1
        assert ar["target_length_sec"] is None
        assert ar["max_duration_sec"] == 30.0
